...
function parseExcelData() {
    ...
    for (let line of lines) {
        ...
        // Tab veya birden fazla boşlukla ayrılmış verileri parçala
        const parts = line.split(/[\t\s]+/);
        
        if (parts.length >= 4) {
            const number = parts[0].trim();
            // İlk ve son parçalar dışındakileri ad olarak birleştir
            const name = parts.slice(1, -2).join(' ').trim();
            const book1Points = parseInt(parts[parts.length - 2]) || 0;
            const book2Points = parseInt(parts[parts.length - 1]) || 0;

            // Aynı numaralı öğrenci var mı kontrol et
            if (!students.some(student => student.number === number)) {
                students.push({
                    number: number,
                    name: name,
